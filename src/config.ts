...
const createConfig = (
  ...
  const apiRoot = envApiRoot(`${aggregatorId}_API_BASE_URL`, fallbackApiRoot);
  const apiKey = envOrNull(`${aggregatorId}_API_KEY`) || apiKeyDefault;
  const integrator = envOrNull(`${aggregatorId}_INTEGRATOR`) || integratorDefault || "astrolab";
  const referrer = envOrNull(`${aggregatorId}_REFERRER`) || referrerDefault;
  const feeBps = envInt(`${aggregatorId}_FEE_BPS`, feeBpsDefault);
